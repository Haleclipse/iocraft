...
pub type PageRenderer = Box<dyn Fn() -> AnyElement<'static> + Send + Sync>;
...
impl UIRouterBuilder {
    ...
    where
        F: Fn() -> AnyElement<'static> + Send + Sync + 'static,
    {
...
mod tests {
    ...
    fn builder_basic() {
        let app = UIRouterBuilder::new()
            .route("home", "Home", || element!(Text(content: "H")).into())
            .route("about", "About", || element!(Text(content: "A")).into())
            .default("home")
            .build()
            .expect("build");
