...
fn css_with_cell_units(s: &str) -> String {
    static BARE_NUMBER_RE: OnceLock<regex::Regex> = OnceLock::new();
    static REPEAT_COUNT_RE: OnceLock<regex::Regex> = OnceLock::new();
    ...
    let re = BARE_NUMBER_RE
        .get_or_init(|| regex::Regex::new(r"([a-zA-Z_-]?)(\d+(?:\.\d+)?)([a-zA-Z%]*)").unwrap());
    ...
    // `repeat(3px,` → `repeat(3,`.
    let re2 = REPEAT_COUNT_RE.get_or_init(|| regex::Regex::new(r"repeat\(\s*(\d+)px\s*,").unwrap());
