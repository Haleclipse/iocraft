...
fn grapheme_width(grapheme: &str) -> usize {
    ...
    // Single visible codepoint in a multi-codepoint cluster: this typically means a
    // base character + VS16 (emoji presentation selector). Terminals render the emoji
    // presentation as 2 columns even when the base character's East Asian Width says 1.
    // Example: ☀ (U+2600, EAW:Ambiguous → 1) + VS16 → ☀️ (2 columns on screen).
    if total_cps > 1 {
        return 2;
    }

    // Single codepoint, single cluster: use its East Asian Width.
    first_visible_width
}
...
",
    ...
    fn write_row_impl<W: Write>(&self, y: usize, mut w: W, ansi: bool) -> io::Result<()> {
        ...
        while col < row.len() {
            ...
            // Write the character BEFORE the end-of-line clear so that
            // the last cell retains its style (invert, underline, etc.).
            if let Some(c) = &cell.character {
            ...
            if ansi && col >= self.width {
                // Reset attributes that would bleed into the cleared area.
                // CSI K fills the erased region with the current SGR state,
                // so any active style or hyperlink must be closed first.
