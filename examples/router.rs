...
fn home_page() -> AnyElement<'static> {
    element! {
...
fn about_page() -> AnyElement<'static> {
    element! {
        View(flex_direction: FlexDirection::Column, padding: 1) {
...
fn settings_page() -> AnyElement<'static> {
    element! {
        View(flex_direction: FlexDirection::Column, padding: 1) {
...
fn App(mut hooks: Hooks) -> impl Into<AnyElement<'static>> {
    ...
    let router_handle =
        ReactiveRouterHandle::new_with_hooks(&mut hooks, app.config.clone()).expect("router init");
